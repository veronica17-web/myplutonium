...
const bookModel = require("../models/bookModel")
    ...
    let createEntry= await bookModel.create(getdata)
    res.send({msg:createEntry})
    }


    let getenteries = async function(req,res){
        let booksList = await bookModel.find().select({ bookName:1 , authorName: 1,_id:0})
        res.send({data:booksList})
...
let getParticularBooks = async function(req,res){
    let perticularEntry = req.body
    let data = await bookModel.find(perticularEntry)
    res.send({msg : data})
}

    
let  getDateOnInr = async function(req,res){
    let dataINR =await bookModel.find({"prices.indianPrice": {$in:["100INR","200INR","500INR"]}})
res.send({data:dataINR})  
}
...
//find({"prices.indianPrice" : {$in :["200INR" , "300INR" , "800INR"]}} )
...
module.exports.getbooks = getenteries
module.exports.yrbooks = getbooksinyr
module.exports.getDateOnInr =getDateOnInr
module.exports.getRandomBook = randomBooks
